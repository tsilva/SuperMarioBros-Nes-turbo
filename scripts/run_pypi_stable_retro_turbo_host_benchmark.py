#!/usr/bin/env python3
"""Run and cache the PyPI stable-retro-turbo host baseline."""
...
def make_remote_run_name(version: str, workload_hash: str) -> str:
    ...
    return f"host-pypi-stable-retro-turbo-{safe_version}-{stamp}-{workload_hash[:8]}"
...
def remote_setup(args: argparse.Namespace, remote: PurePosixPath, version: str) -> None:
    ...
    run(
        [
            ...
            "-e",
            rsync_rsh(args),
            "scripts/benchmark_stable_retro_turbo_pypi.py",
...
def aggregate(args: argparse.Namespace, cache_dir: Path, remote: PurePosixPath, version_info: dict[str, Any], workload_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    aggregate_payload = {
        "mode": "pypi_stable_retro_turbo_fixed_host",
