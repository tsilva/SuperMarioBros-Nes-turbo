...
def source_contract(version: str) -> tuple[str, str]:
    project = tomllib.loads((REDIRECT_ROOT / "pyproject.toml").read_text())[
        "project"
    ]
    ...
    setuptools = tomllib.loads((REDIRECT_ROOT / "pyproject.toml").read_text()).get(
        "tool", {}
    ).get("setuptools", {})
...
def audit_dist(dist: Path, version: str) -> None:
    ...
    unexpected = [path.name for path in all_files if path not in files and path.name != ".gitignore"]
    if unexpected:
