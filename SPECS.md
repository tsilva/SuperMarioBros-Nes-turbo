...
Provide a throughput-first Super Mario Bros NES RL environment specialized for the supported SMB/NROM workload while preserving the Python contracts needed for SB3 training, stable-retro-turbo parity, playback, benchmarking, and release validation.
...
- Preserve SB3 `VecEnv` behavior for spaces, `reset()`, `step_async()`, `step_wait()`, infos, autoreset, terminal observations, and `step_wait_gymnasium()` terminated/truncated arrays.
...
- Preserve native game-over and flag-completion termination plus additive `done_on`/`done_on_info` semantics, named events, `change`/`increase`/`decrease`, `info["done_on_info"]`, and terminal observations.
...
- Preserve parity/regression tests for SB3 VecEnv behavior, native/stable-retro constructor compatibility, state sampling, terminal infos, sticky/no-op actions, benchmark stats, release flow, and benchmark metadata.
