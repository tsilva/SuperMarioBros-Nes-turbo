...
- Keep `train.py` as a simple observation-free JERK action-sequence search starting from `Level1-1`; training episodes must end on life loss, must not end on level change, and its native checkpoints must remain playable without a neural-network framework.
